...
class RequisitionController extends Controller
{
    ...
    public function markOnHand($id, Request $request) { $validator = Validator::make($request->all(), ['warehouse_location' => 'required|string|max:100', 'payments' => 'required|array|min:1', 'payments.*.company_account_id' => 'required|integer|distinct|exists:company_accounts,id', 'payments.*.amount' => 'required|numeric|min:0.01']); if ($validator->fails()) return $this->respond(false, 'Validation error', $validator->errors(), 422); try { $data = $validator->validated(); $record = $this->service->markOnHand($id, $data['warehouse_location'], $data['payments'], $request->user()?->id, $request->ip()); return $record ? $this->respond(true, 'Procurement is on hand, inventory updated, and payment recorded.', $record) : $this->respond(false, 'Procurement is not available to mark on hand.', [], 422); } catch (\InvalidArgumentException $e) { return $this->respond(false, $e->getMessage(), [], 422); } }
