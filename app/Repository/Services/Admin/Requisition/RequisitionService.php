...
class RequisitionService
{
    ...
    public function stockOptions() { return ['products' => DB::table('products')->orderBy('name')->get(['id', 'name', 'sku']), 'warehouses' => DB::table('inventory')->whereNotNull('warehouse_location')->where('warehouse_location', '!=', '')->distinct()->orderBy('warehouse_location')->pluck('warehouse_location')->values()]; }
    public function productStocks($perPage, $page, $search) { return DB::table('inventory as i')->join('products as p', 'p.id', '=', 'i.product_id')->select('i.id', 'i.product_id', 'i.warehouse_location', 'i.stock_quantity', 'p.name as product_name', 'p.sku')->where(fn ($q) => $q->where('p.name', 'like', "%{$search}%")->orWhere('p.sku', 'like', "%{$search}%")->orWhere('i.warehouse_location', 'like', "%{$search}%"))->orderBy('p.name')->orderBy('i.warehouse_location')->paginate($perPage, ['*'], 'page', $page); }
    public function inventoryAdjustments($perPage, $page, $search) { return DB::table('inventory_adjustments as ia')->join('products as p', 'p.id', '=', 'ia.product_id')->leftJoin('users as u', 'u.id', '=', 'ia.adjusted_by')->select('ia.*', 'p.name as product_name', 'p.sku', 'u.name as adjusted_by_name')->where(fn ($q) => $q->where('p.name', 'like', "%{$search}%")->orWhere('p.sku', 'like', "%{$search}%")->orWhere('ia.warehouse_location', 'like', "%{$search}%")->orWhere('ia.reason', 'like', "%{$search}%")->orWhere('u.name', 'like', "%{$search}%"))->orderByDesc('ia.id')->paginate($perPage, ['*'], 'page', $page); }
    public function inventoryAdjustment($id) { return DB::table('inventory_adjustments as ia')->join('products as p', 'p.id', '=', 'ia.product_id')->leftJoin('users as u', 'u.id', '=', 'ia.adjusted_by')->where('ia.id', $id)->first(['ia.*', 'p.name as product_name', 'p.sku', 'p.price as product_price', 'u.name as adjusted_by_name', 'u.email as adjusted_by_email']); }
    public function saveProductStock(array $data, $userId = null) { return DB::transaction(function () use ($data, $userId) { $stock = DB::table('inventory')->where('product_id', $data['product_id'])->where('warehouse_location', $data['warehouse_location'])->lockForUpdate()->first(); $previous = $stock->stock_quantity ?? 0; if ($stock) DB::table('inventory')->where('id', $stock->id)->update(['stock_quantity' => $data['stock_quantity']]); else { $id = DB::table('inventory')->insertGetId(['product_id' => $data['product_id'], 'warehouse_location' => $data['warehouse_location'], 'stock_quantity' => $data['stock_quantity']]); $stock = (object) ['id' => $id]; } $this->recordAdjustment($stock->id, $data['product_id'], $data['warehouse_location'], $previous, $data['stock_quantity'], $data['reason'] ?? 'Opening stock', $userId); return DB::table('inventory as i')->join('products as p', 'p.id', '=', 'i.product_id')->where('i.id', $stock->id)->first(['i.id', 'i.product_id', 'i.warehouse_location', 'i.stock_quantity', 'p.name as product_name', 'p.sku']); }); }
    public function adjustProductStock($id, array $data, $userId = null) { return DB::transaction(function () use ($id, $data, $userId) { $stock = DB::table('inventory')->where('id', $id)->lockForUpdate()->first(); if (!$stock) return null; DB::table('inventory')->where('id', $id)->update(['stock_quantity' => $data['stock_quantity']]); $this->recordAdjustment($id, $stock->product_id, $stock->warehouse_location, $stock->stock_quantity, $data['stock_quantity'], $data['reason'] ?? 'Manual stock adjustment', $userId); return DB::table('inventory as i')->join('products as p', 'p.id', '=', 'i.product_id')->where('i.id', $id)->first(['i.id', 'i.product_id', 'i.warehouse_location', 'i.stock_quantity', 'p.name as product_name', 'p.sku']); }); }
    private function recordAdjustment($inventoryId, $productId, $warehouse, $previous, $current, $reason, $userId) { DB::table('inventory_adjustments')->insert(['inventory_id' => $inventoryId, 'product_id' => $productId, 'warehouse_location' => $warehouse, 'previous_quantity' => $previous, 'new_quantity' => $current, 'adjustment_quantity' => $current - $previous, 'reason' => $reason, 'adjusted_by' => $userId, 'created_at' => now(), 'updated_at' => now()]); }
    private function stockReceiptQuery() { return DB::table('stock_receipts as sr')->join('products as pr', 'pr.id', '=', 'sr.product_id')->join('procurements as p', 'p.id', '=', 'sr.procurement_id')->leftJoin('users as receiver', 'receiver.id', '=', 'sr.received_by')->select('sr.*', 'pr.name as product_name', 'pr.sku as product_sku', 'p.procurement_number', 'receiver.name as received_by_name'); }
